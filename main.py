...
class Parser:
    ...
    def parse_main_page(self, url: str, sport_country: tuple[str, str]) -> list:
        ...
        try:
            ...
            logger.info(f"Найдено матчей: {matches}")
    ...
    def parse_match(self, block: BeautifulSoup, sport_country: tuple[str, str], liga: str) -> list:
        try:
            ...
            logger.info(f"Парсинг матча {match_data}")
            return match_data
    ...
    def parse_match_page(self, link: str) -> str:
        try:
            ...
            logger.info(f"Парсинг таблицы 'забьют гол': {result['Match will score a goal']}")
            ...
            for stat_type in ['Угловые', 'Фолы']:
                try:
                    ...
                    logger.info(f"Парсинг таблицы '{stat_type}': {result[f'{prefix} individual total']}")
                    ...
                    logger.info(f"Парсинг таблицы '{stat_type} 1-го тайма': {result[f'{prefix} Individual total 1st half']}")
